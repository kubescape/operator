...
docker build --no-cache -f Dockerfile.test -t quay.io/armosec/k8s-ca-websocket:$WTAG .
rm -rf k8s-ca-websocket
# docker push quay.io/armosec/k8s-ca-websocket:$WTAG
...
kubectl -n cyberarmor-system set image deployment/ca-websocket ca-websocket-container=quay.io/armosec/k8s-ca-websocket:$WTAG || true
